...
Shader "Fx/Effect/Mask" {
Properties {
	_MainTex ("Base layer (RGB)", 2D) = "white" {}
	_DetailTex ("2nd layer (RGB)", 2D) = "white" {}
	_AphlaTex ("3nd layer (A)", 2D) = "white" {}
...
SubShader {
	...
	v2f vert (appdata_full v)
	{
		...
		o.orguv = TRANSFORM_TEX(v.texcoord.xy,_MainTex);
		o.uv.xy = o.orguv.xy + frac(float2(_ScrollX, _ScrollY) * _Time);
	...
	Pass {
		...
		fixed4 frag (v2f i) : COLOR
		{
			...
			fixed4 tex3 = tex2D (_AphlaTex, i.orguv);
